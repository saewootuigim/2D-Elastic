subroutine Gauss_Legendre( N, x, w ) ! Gauss-Legendre sample Points ad Weights
	...
	integer*4 :: i, j
	integer*4 :: N1, N2
	real*8, dimension(N) :: xu, y0, Lp
	real*8, dimension(N,N+1) :: L

	N1 = N - 1
	N2 = N + 1
	
	! Form xu. xu contains N equally spaced points between -1 and 1, including the end points.
	do i = 1, N
		xu(i) = -1d0 + (i-1)*2d0/(N-1)
	end do

	! Initial guess
	do i = 1, N
		x(N2-i) = cos((2d0*(i-1)+1d0)*PI/(2d0*N1+2))+.27d0/N*sin(pi*xu(i)*N1/N2)
	end do

	! Compute the zeros of the N Legendre polynomial using the recursion relation and the Newton-Raphson method.
	y0 = 2d0

	do while( maxval(abs(x-y0))>epsilon(0d0) )
		L(:,1) = 1d0
		Lp = 0d0
		L(:,2) = x
		do i = 2, N
			do j = 1, N
				L(j,i+1) = ((2d0*i-1d0)*x(j) * L(j,i)-dble(i-1)*L(j,i-1))/dble(i)
			end do
		end do

		do i = 1, N
			Lp(i) = N2 * (L(i,N)-x(i)*L(i,N2))/dble(1-x(i)**2)
		end do

		y0 = x

		do i = 1, N
			x(i) = y0(i) - L(i,N2)/Lp(i)
		end do
	end do

	! Compute the weights.
	do i = 1, N
    	w(i) = 2d0*(dble(N2)/dble(N))**2/( (1d0-x(i)**2) * Lp(i)**2 )
	end do

end subroutine Gauss_Legendre

subroutine Gauss_Kronrod ( nk, xk, wk_out )

!*****************************************************************************80
!
!  WARNING: Algorithm used here can only compute Gauss-Kronrod quadrature
!           nodes up to 77. If more nodes are requested, the iteration
!           will not converge.
!
...
!
  ...
  real*8, dimension(nk), intent(out) :: xk, wk_out
  ...
  integer ( kind = 4 ) m
  real ( kind = 8 ) s
  real ( kind = 8 ), allocatable, dimension(:) :: b, tau, wk, wl, x
  real ( kind = 8 ) x1
  real ( kind = 8 ) xx
  real ( kind = 8 ) y
  ...
  if( mod(nk,2)==0 ) then
  	print '(a)', 'ERROR **************************************************************************'
  	print '(a)', 'Gauss-Kronrod quadrature always have odd number nodes'
  	print '(a)', 'and weights. Requested number of nodes and weights is'
  	print '(1x,i4,a1)', nk, '.'
  	stop
  endif
...
!
  do k = 1, n, 2
    ...
    call abwe1 ( n, m, eps, coef2, even, b, xx, wk(k) )
    wl(k) = 0.0D+00
...
!
    call abwe2 ( n, m, eps, coef2, even, b, xx, wk(k+1), wl(k+1) )
...
!
  if ( even ) then
    xx = 0.0D+00
    call abwe1 ( n, m, eps, coef2, even, b, xx, wk(n+1) )
    wl(n+1) = 0.0D+00
    x(n+1) = xx
  end if

!
! Re-order the output array to my taste.
! Original code makes x in descending order with only the positive numbers because they are symmetric.
! I want to expand them to be in ascending order from -1 to 1.
!
	do k = 1, n
		xk(k) = -x(k)
		wk_out(k) = wk(k)
	...
	do k = 1, n
		xk(n+1+k) = x(n+1-k)
		wk_out(n+1+k) = wk(n+1-k)
	enddo

	deallocate( b, tau, wk, wl, x )
...
subroutine abwe2 ( n, m, eps, coef2, even, b, x, wk, wl )
...
!
  ...
  if ( even ) then
    wk = wl + coef2 / ( pd2 * x * ( p2 - p1 ) )
  else
    wk = wl + 2.0D+00 * coef2 / ( pd2 * ( p2 - p0 ) )
  end if
